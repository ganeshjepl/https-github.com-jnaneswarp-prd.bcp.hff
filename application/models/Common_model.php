...
//      echo  $sql = $this->db->last_query(); exit;
	...
	public function insert_update_onduplicate_data($update_array = array(),$autoIncermentArray = array()) {
		$onUpdateStr = '';
		$onUpdateStr = ' ON DUPLICATE KEY UPDATE ';
		if(count($autoIncermentArray) > 0) {
			
			foreach($autoIncermentArray as $aikey => $aival) {
				$onUpdateStr .= $aikey ."=(".$aikey."+".$aival."),";
			}
			
		}
		if(count($update_array) > 0) {
			$onUpdateStr = rtrim($onUpdateStr,",");
			foreach($update_array as $ukey => $uval) {
				$onUpdateStr .= $ukey ."='".$uval."',";
			}
			$onUpdateStr = rtrim($onUpdateStr,",");
		}else {
			$onUpdateStr .= " mts = now() ";
		...
		$sql = $this->db->insert_string($this->dbTable, $this->insertUpdateArray) .$onUpdateStr;
		$this->db->query($sql);
    ...
    public function join($table,$condition){
        ...
        $this->db->join($table,implode('=', $condition),'INNER');
    }
